...
➡️ [**`ExceptionHandler`**](https://rubygems.org/gems/exception_handler) ⬅️ was designed to replace Rails' *static* error pages with **DYNAMIC** views (utilizing the views/layouts/assets in your app). The secret is to **understand how HTTP works** (from an "errors" perspective)...

<p align="center">
  <img src="./readme/http_codes.png" />
</p>

**⛔ HTTP ERRORS don't mean *anything* on their own ⛔** - they are [**STATUS CODES**](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes) - designed to provide the client (typically a "browser") with a [`message_body`](https://en.wikipedia.org/wiki/HTTP_message_body) generated by the server. What you see is **NOT** the error - but the "message" that was generated to explain what the problem was.

*How* that message is generated has very little to do with HTTP, or even your backend software. It's *entirely* dependent on your application, and the processes it has to *capture* any errors it may craete, and turn them into a valid HTTP response.

**HTTP is a means of *communication*** - meaning that your browser sends a *request* and the HTTP server provides a *response*. Known as a [`stateless protocol`](https://en.wikipedia.org/wiki/Stateless_protocol), the "HTTP" element of the transaction is a means of transferring data between two Internet-connected systems.
