...
**⛔ HTTP ERRORS don't mean *anything* on their own ⛔** → they are [**STATUS CODES**](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes) designed to provide the client with a [`message_body`](https://en.wikipedia.org/wiki/HTTP_message_body) generated by the server. What you see is **NOT** the error - but the "message" that was generated to explain what the problem was.

The point is that what you *SEE* on screen has almost nothing to do with Rails, your web server or browser. It's an HTTP response to an HTTP request. What goes into the

This means that if you're looking to "custmize"
